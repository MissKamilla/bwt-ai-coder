...
@app.get("/api/board")
async def get_board(_user: str = Depends(require_user)) -> dict:
    _ensure_db()
...
async def serve_static(static_path: str) -> Response:
    if not STATIC_DIR.is_dir():
        return Response("static build not found", status_code=404, media_type="text/plain")

    root = STATIC_DIR.resolve()
    target = (root / static_path).resolve()
